...
def get_ckpt_name(config):
    if config.get("use_internal_controller", False):
        return f'shapenet_model_cls.pth'
    return f'shapenet_model_{config["llm"]}.pth'
    ...
    for epoch in tqdm(range(config["shapenet_epoch"])):
        ...
        for _, batch in enumerate(dloader):
            ...
            for t in range(T):
                ...
                # For original DynaOD, these are external controls.
                # For DynaOD_CLS, they will be ignored by forward().
        ...
        val_metrics, _ = test_process(config, val_loader, model)
...
def test_process(
    ...
    with torch.no_grad():
        for _, batch in tqdm(enumerate(dloader)):
            geoids, bgs_by_day, batched_dis, ods_by_day = batch
            T = len(bgs_by_day)
            ...
            for t in range(T):
                ...
                # external shapes only used in ablation / retrieval-based inference
...
if __name__ == "__main__":
    ...
    model_config = {
        ...
        # ========== controller setting ==========
        "use_internal_controller": True,   # False: original DynaOD, True: DynaOD_CLS
        ...
        "lambda_smooth": 0.1,
        "alpha_stable": 2,
        "llm": "qwen-2.5-1.5b-sft",  # still used for naming / original control source
    ...
    # trainable params
    trainable_params = list(dyna_model.shape_net.parameters())
    if model_config["use_internal_controller"]:
        trainable_params += list(dyna_model.directional_controller.parameters())

    optimizer = torch.optim.AdamW(trainable_params, lr=model_config["shapenet_lr"])
