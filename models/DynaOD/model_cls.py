...
class MLPDirectionalController(nn.Module):
    """
    A non-LLM baseline controller.

    Input:
      poi_feat:      (N, poi_dim)
      demo_feat:     (N, demo_dim)
      t_context_seq: (N, T, t_dim)

    Output:
      poi_ctrl_seq:  (N, T, poi_dim)  in [-1, 1]
      demo_ctrl_seq: (N, T, demo_dim) in [-1, 1]

    We predict 3-class logits for each feature dimension:
      0 -> decrease (-1)
      1 -> stable   ( 0)
      2 -> increase (+1)

    Then convert to soft directional value:
      ctrl = p(increase) - p(decrease)
    """

    def __init__(self, poi_dim=34, demo_dim=97, t_dim=8, hidden=256):
        super().__init__()
        self.poi_dim = poi_dim
        self.demo_dim = demo_dim
        self.t_dim = t_dim

        in_dim = poi_dim + demo_dim + t_dim
        ...
        self.poi_head = nn.Linear(hidden, poi_dim * 3)
        self.demo_head = nn.Linear(hidden, demo_dim * 3)
    ...
    @staticmethod
    def logits_to_soft_ctrl(logits: torch.Tensor) -> torch.Tensor:
        """
        logits: (..., D, 3)
        return: (..., D) in [-1, 1]
        """
        probs = torch.softmax(logits, dim=-1)
        # decrease=-1, stable=0, increase=+1
        return probs[..., 2] - probs[..., 0]

    def forward(self, poi_feat, demo_feat, t_context_seq):
        N, T, _ = t_context_seq.shape

        static_feat = torch.cat([demo_feat, poi_feat], dim=-1)      # (N, D_demo + D_poi)
        static_feat = static_feat.unsqueeze(1).expand(-1, T, -1)    # (N, T, D)

        x = torch.cat([static_feat, t_context_seq], dim=-1)         # (N, T, D + t_dim)
        h = self.shared(x)                                          # (N, T, hidden)

        poi_logits = self.poi_head(h).view(N, T, self.poi_dim, 3)
        demo_logits = self.demo_head(h).view(N, T, self.demo_dim, 3)

        poi_ctrl_seq = self.logits_to_soft_ctrl(poi_logits)         # (N, T, poi_dim)
        demo_ctrl_seq = self.logits_to_soft_ctrl(demo_logits)       # (N, T, demo_dim)

        return {
            "poi_ctrl_seq": poi_ctrl_seq,
            "demo_ctrl_seq": demo_ctrl_seq,
            "poi_logits_seq": poi_logits,
            "demo_logits_seq": demo_logits,
        }
...
class DynaOD(nn.Module):
    def __init__(self, diff_config, pretrained_ckpt=None, freeze_diffusion=True,
                 ...
                 use_internal_controller: bool = False,
                 controller_hidden: int = 256,
                 t_dim: int = 8):
        super().__init__()
        self.diff_config = diff_config
        self.diffusion_net = Diffusion(diff_config)
        self.shape_net = ShapeNet(poi_dim=poi_dim, demo_dim=demo_dim, t_dim=t_dim)
        self.poi_dim = poi_dim
        self.demo_dim = demo_dim
        self.t_dim = t_dim
        self.use_internal_controller = use_internal_controller

        if self.use_internal_controller:
            self.directional_controller = MLPDirectionalController(
                poi_dim=poi_dim,
                demo_dim=demo_dim,
                t_dim=t_dim,
                hidden=controller_hidden,
            )
            print("[DynaOD] Using internal MLP directional controller.")
        else:
            self.directional_controller = None
    ...
    def forward(self, poi_ctrl_list, demo_ctrl_list, t_context_list, external_poi_shp, external_demo_shp,
        ...
        if self.use_internal_controller:
            ctrl_out = self.directional_controller(poi_feat, demo_feat, t_context_seq)
            poi_ctrl_seq = ctrl_out["poi_ctrl_seq"]       # (N,T,poi_dim), soft in [-1, 1]
            demo_ctrl_seq = ctrl_out["demo_ctrl_seq"]     # (N,T,demo_dim), soft in [-1, 1]
        else:
...
class ShapeNet(nn.Module):
    ...
    def _sample_and_aggregate(self, mu: torch.Tensor, logvar: torch.Tensor, weight_logits: torch.Tensor | None):
        ...
        std = torch.exp(0.5 * logvar).unsqueeze(2)
        samples = mu.unsqueeze(2) + eps * std

        if self.agg_mode == "mean":
            return samples.mean(dim=2), None

        w = torch.softmax(weight_logits, dim=-1).unsqueeze(-1)
        agg = (samples * w).sum(dim=2)
        return agg, w.squeeze(-1)
    ...
    def weighted_smoothness_loss(shape_seq: torch.Tensor,
        ...
        """
        Compatible with both hard controls {-1,0,1} and soft controls in [-1,1].
        """
        diff = shape_seq[:, 1:, :] - shape_seq[:, :-1, :]
        stable = (ctrl_seq[:, :-1, :].abs() < 0.1).float()
        w = 1.0 + alpha * stable
    ...
    def forward(self, poi_feat, demo_feat, poi_ctrl_seq, demo_ctrl_seq, t_context_seq):
        ...
        poi_on = (poi_ctrl_seq.abs() > 1e-6).float()
        demo_on = (demo_ctrl_seq.abs() > 1e-6).float()
        ...
        poi_h = F.relu(self.poi_token_proj(poi_token))
        demo_h = F.relu(self.demo_token_proj(demo_token))

        if self.use_gru:
            poi_h, _ = self.poi_gru(poi_h)
            demo_h, _ = self.demo_gru(demo_h)

        h = self.fuse(torch.cat([poi_h, demo_h], dim=-1))

        if self.use_feat_cond:
            z = self.zone_code(torch.cat([poi_feat, demo_feat], dim=-1))
        ...
        weight_logits = None
        if self.agg_mode == "mixture":
            weight_logits = self.sample_weight(h)
