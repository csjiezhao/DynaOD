...
def prepare_test_data(config, external_scalers=None):
    ...
    cities = os.listdir(config["data_path"])[:100]
    ...
    for city in tqdm(cities):
        demos = np.load(config["data_path"] + city + "/demos.npy")
        pois = np.load(config["data_path"] + city + "/pois.npy")
        dis = np.load(config["data_path"] + city + "/dis.npy")

        od = OD_normer.fit_transform(np.load(
            os.path.join(config["data_path"], city, 'ods/2019_01_03_od.npy')
        ))
        one = {
            "GEOID": city,
    ...
    scaler_feat = external_scalers.get("feat", None)
    scaler_dis = external_scalers.get("dis", None)
    scaler_od = external_scalers.get("od", None)

    def apply_scaler(dataset, sf, sd, so):
        for i, one in enumerate(dataset):
            if sf is not None:
                one["nfeat"] = sf.transform(one["nfeat"])
            if sd is not None:
                one["dis"] = sd.transform(one["dis"].reshape(-1, 1)).reshape(one["dis"].shape)
            if so is not None:
                one["od"] = so.transform(one["od"].reshape(-1, 1)).reshape(one["od"].shape)

    apply_scaler(data, scaler_feat, scaler_dis, scaler_od)
