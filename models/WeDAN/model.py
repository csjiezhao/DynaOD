...
class DenoisingNet(nn.Module):
    ...
    def forward(self, inputs, masks, dis, t, batchlization):
        ...
        # time embedding
        if len(t) == 1:
            t_emb = timestep_embedding(self.config, t, dim=self.config["hiddim"], max_period=self.config["T"],
                                       batchlization=batchlization)
        else:
            t_emb_for_n, t_emb_for_e = timestep_embedding(self.config, t, dim=self.config["hiddim"],
                                                          max_period=self.config["T"], batchlization=batchlization)
...
class Diffusion(nn.Module):
    ...
    def q_sample(self, net, t, batchlization):
        ...
        if len(t) == 1:
            ...
            for idx, i in enumerate(recover_od_shapes(batchlization)):
                ...
                noisy_e[l:r, l:r] = sqrt_alphas_cumprod_t * e[l:r, l:r] + sqrt_one_minus_alphas_cumprod_t * noise_e[l:r,
                                                                                                            l:r]
    ...
    def loss(self, net, masks, t, dis, batchlization):
        ...
        input_e = torch.cat(
            [(e * mask_e * batchlization).unsqueeze(-1), (noisy_e * (1 - mask_e) * batchlization).unsqueeze(-1)],
            dim=-1)
        ...
        noise_hat = torch.cat([(noise_n_hat * (1 - mask_n))[mask_n == 0].reshape([-1]),
                               (noise_e_hat * (1 - mask_e) * batchlization)[eloss_idx].reshape([-1])])
        noise = torch.cat([(noise_n * (1 - mask_n))[mask_n == 0].reshape([-1]),
                           (noise_e * (1 - mask_e) * batchlization)[eloss_idx].reshape([-1])])
    ...
    def DDIM_sample(self, cur_net, t, t_pre, c):
        ...
        input_e = torch.cat(
            [(real_e * mask_e * batchlization).unsqueeze(-1), (cur_e * (1 - mask_e) * batchlization).unsqueeze(-1)],
            dim=-1)
        inputs = (input_n, input_e)
        ...
        sigma_t = self.config["DDIM_eta"] * ((1 - self.alphas_cumprod[t_pre]) / (1 - self.alphas_cumprod[t]) * (
                    1 - self.alphas_cumprod[t] / self.alphas_cumprod[t_pre])).sqrt()
        coef_noise_hat = (1 - self.alphas_cumprod[t_pre] - sigma_t ** 2).sqrt()

        # reparameterazation
        n_t_minus_1 = coef_net0_hat * n0_hat + coef_noise_hat * noise_n_hat + torch.randn_like(cur_n,
                                                                                               device=cur_n.device) * sigma_t
        e_t_minus_1 = coef_net0_hat * e0_hat + coef_noise_hat * noise_e_hat + torch.randn_like(cur_e,
                                                                                               device=cur_n.device) * sigma_t * batchlization
        net_t_minus_1 = (n_t_minus_1, e_t_minus_1)

        return net_t_minus_1

    def DDIM_sample_loop(self, shape_n, shape_e, c, return_trace=False):
        # block matrix mask
        _, _, _, batchlization = c
        # DDIM setting
        ...
        # init pure noises
        cur_x = torch.randn(shape_n, device=batchlization.device)
        cur_e = torch.randn(shape_e, device=batchlization.device)
        cur_net = (cur_x, cur_e)

        if return_trace:
            denoising_seq = [(cur_x.detach().cpu(), cur_e.detach().cpu())]

        # denoising
        for t, t_pre in zip(reversed(sample_Ts), reversed(sample_Ts_pre)):
            t = torch.LongTensor([t]).to(batchlization.device)
            t_pre = torch.LongTensor([t_pre]).to(batchlization.device)
            ...
            if return_trace:
                cur_x, cur_e = cur_net
                denoising_seq.append((cur_x.detach().cpu(), cur_e.detach().cpu()))
