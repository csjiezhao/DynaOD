...
def call_one_tract(tract_id: str, date: str, platform: str, model: str) -> list[int]:
    """返回 34 维 [-1,0,1] 列表"""
    llm = get_thread_llm(platform, model)
    prompt = POI_CTRL_PROMPT.format(TRACT=tract_id, DATE=date)
    try:
        ...
        vec = []
    return (vec[:34] + [0] * (34 - len(vec)))[:34]


def process_city_date(city_id: str, date: str, args, city2tract: dict, pool: ThreadPoolExecutor):
    save_file = Path(args.data_path) / city_id / "poi_vecs" / f"{args.model}_poi_vec_{date}.npy"
    if save_file.exists() and not args.overwrite:
        tqdm.write(f"⏩ Skip  {city_id}  {date}")
        return
    ...
    # 提交任务：future -> index
    futures = {
        pool.submit(call_one_tract, tid, date, args.platform, args.model): i
...
def main():
    ...
    cities, dates = load_samples_ijcai(args.data_path, True, 0.7, args.mode, True)

    tasks = [(c, d) for c in cities for d in dates]

    # ✅ 全局复用线程池（关键加速点）
    with ThreadPoolExecutor(max_workers=args.parallel) as pool:
        for city_id, date in tqdm(tasks, desc="city-date"):
